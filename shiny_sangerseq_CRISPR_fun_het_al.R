...
phaseShift_function <- function(both_alleles, input_reference, beginning_start, homo_match_len, offset_3p, outp1, input_file, sangerobj){
  ...
  rev_ref <- reverseComplement(DNAString(reference))
  ...
  if (length(match_fw) == 0 && length(match_rev) == 0) {
    ...
    if (length(match_fw) == 1) {
      refseq <- DNAString(reference)
      beginning_match <- match_fw
    }   
    else if (length(match_rev) == 1) {
      refseq <- DNAString(rev_ref)
    ...
    if(num_hom > 0.8) {
      aligned_str <- pairwiseAlignment(DNAString(both_alleles_str), refseq, type="local-global") #local-global doesn't like end gaps
      ...
      end_matches <- matchPattern(DNAString(both_alleles_str, start = start_pos, match_str_len), refseq, fixed = F, 
      ...
      while (match_str_len > 15) {
        if (length(end_matches) == 2) {
          ...
          print(as.character(refseq), quote = F)
          print(end_matches)
          ...
          if (0 %in% alleles)
          {hetseqalleles <- setAllelePhase(sangerobj, refseq, trim5 = 60, trim3 = 80)
